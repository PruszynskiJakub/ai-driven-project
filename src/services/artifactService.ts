...
export async function addFeedbackAndIterate(artifactId: string, data: AddFeedbackRequest): Promise<(ArtifactWithVersionResponse & {
...
export async function updateArtifactContent(artifactId: string, data: UpdateArtifactContentRequest): Promise<(ArtifactWithVersionResponse & {
    newVersionCreated: boolean
}) | null> {
    const [artifact] = await db.select().from(artifacts).where(eq(artifacts.id, artifactId));
